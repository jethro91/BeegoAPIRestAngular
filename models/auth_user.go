...
type AuthUser struct {
	...
	First    string    `orm:"column(first);size(255)"`
	Last     string    `orm:"column(last);size(255)"`
	Email    string    `orm:"column(email);size(255)"`
	Password string    `orm:"column(password);size(255)"`
	RegKey   string    `orm:"column(reg_key);size(255)"`
	RegDate  time.Time `orm:"column(reg_date);type(datetime)"`
}
